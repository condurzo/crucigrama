...
- (void)postStatusUpdate:(NSString*)status withImageAtPath:(NSString*)path
{
	...
	if( !tokenString )
	{
		UnitySendMessage( "TwitterManager", "twitterRequestDidFail", "User is not logged in" );
		return;
	}
	...
	^{
		...
		^{
			...
			{
				UnitySendMessage( "TwitterManager", "twitterRequestDidFail", "Unknown error. No data returned and no network error occurred" );
			}
		});
	});
}
